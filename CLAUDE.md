...
**Password reset needs no schema.** `POST /api/password/forgot` → `POST /api/password/reset`. The token is `HMAC(SESSION_SECRET, "reset:" + payload + current password_hash)` with a 1h expiry, so changing the password invalidates every outstanding link — that's the single-use guarantee, there is no tokens table. Email goes out over SMTP in `sendEmail()` (nodemailer, `smtp.gmail.com` by default, `SMTP_PASS` is a Google **app password**); without `SMTP_USER` it just logs the link, so dev works unconfigured. Switching provider is env-only — no code change. `node scripts/test-email.js <address>` checks the credentials on their own. `npm test` runs `scripts/test-reset-token.js`, the only test in the repo — keep it passing when touching the token helpers.
...
**Syllabus → plan is the core feature.** `SYLLABUS` (base topics per subject) + `SYLLABUS_OVERRIDES` (per-objective, e.g. SAT math) in `app.js` define the exam curriculum. The user rates each topic (Não sei / Estudando / Já domino), stored in `syllabus_progress`; custom topics are just extra rows. The editor also sets a per-topic block length (`syllabus_progress.minutes`, 15–240, NULL = the front's 60min default — the upsert `COALESCE`s it so saving a status alone never wipes it) and deletes topics. **Deleting writes `status = 'removed'` instead of removing the row**, because base topics come from the `SYLLABUS` constant and a deleted row would just reappear; `baseTopics()`/`customTopics()` filter that status out and the editor lists removed topics with a "Restaurar" button. `GET /api/dashboard` also returns `suggestions`: topics with 2h+ of completed study still rated `unknown` (it joins `study_sessions.detail` to `syllabus_progress.topic`, so a free-text "extra study" detail only counts if it matches a real topic). The front shows them one at a time in `#study-alert` (`renderStudyAlert()`) with the subject and topic; **the status only changes when the user confirms** — the server never rewrites it. `renderWeekPlan()` builds the weekly plan by pulling real topics from `pendingQueue()`, prioritizing `unknown > learning > mastered`, and fills the whole free-time window using each topic's own duration (plus a 1h break when free time > 3h). `topicMinutes()` clamps to 15–240 — a 0 would make the day-filling loop spin forever.
